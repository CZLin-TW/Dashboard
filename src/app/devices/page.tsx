    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {(() => {
        ...
        const btnClass = (type: string, value: unknown, isActive: boolean) =>
          `rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${isFailed(type, value) ? "bg-warm text-white animate-pulse" : isPending(type, value) ? "bg-amber-500 text-white animate-pulse" : isActive ? "bg-cool text-white" : "bg-elevated text-soft hover:bg-mute/20"}`;
                  ...
                  <Card className={pin.isDevicePinned(device.name) ? "border-cool/30" : ""}>
                    ...
                    {device.type === "空調" && (() => {
                      ...
                      <div className="space-y-3">
                        ...
                        <div><label className="text-xs text-mute">電源</label><div className="mt-1 flex gap-2"><button onClick={() => updateAcPending(device, { power: true })} className={`rounded-lg px-4 py-1.5 text-sm font-medium transition-colors ${pending.power ? "bg-cool text-white" : "bg-elevated text-soft hover:bg-mute/20"}`}>ON</button><button onClick={() => updateAcPending(device, { power: false })} className={`rounded-lg px-4 py-1.5 text-sm font-medium transition-colors ${!pending.power ? "bg-warm text-white" : "bg-elevated text-soft hover:bg-mute/20"}`}>OFF</button></div></div>
                        <div><label className="text-xs text-mute">溫度</label><div className="mt-1 flex items-center gap-3"><button onClick={() => updateAcPending(device, { temperature: Math.max(options.ac.temperature.min, pending.temperature - 1) })} className="flex h-8 w-8 items-center justify-center rounded-lg bg-elevated hover:bg-mute/20">−</button><span className="w-16 text-center text-xl font-bold">{pending.temperature}°C</span><button onClick={() => updateAcPending(device, { temperature: Math.min(options.ac.temperature.max, pending.temperature + 1) })} className="flex h-8 w-8 items-center justify-center rounded-lg bg-elevated hover:bg-mute/20">+</button></div></div>
                        <div><label className="text-xs text-mute">模式</label><div className="mt-1 flex flex-wrap gap-2">{options.ac.modes.map((m) => (<button key={m.value} onClick={() => updateAcPending(device, { mode: m.value })} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${pending.mode === m.value ? "bg-cool text-white" : "bg-elevated text-soft hover:bg-mute/20"}`}>{m.label}</button>))}</div></div>
                        <div><label className="text-xs text-mute">風速</label><div className="mt-1 flex flex-wrap gap-2">{options.ac.fan_speeds.map((s) => (<button key={s.value} onClick={() => updateAcPending(device, { fanSpeed: s.value })} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${pending.fanSpeed === s.value ? "bg-cool text-white" : "bg-elevated text-soft hover:bg-mute/20"}`}>{s.label}</button>))}</div></div>
                        <button onClick={() => sendAcCommand(device)} disabled={sending} className={`w-full rounded-lg py-2.5 text-sm font-bold transition-colors ${failed ? "bg-warm text-white animate-pulse" : dirty ? "bg-fresh text-white hover:bg-fresh/85" : "bg-elevated text-mute"}`}>{failed ? "失敗，請重試" : sending ? "送出中..." : dirty ? "送出設定" : "未變更"}</button>
                      </div>
                      );
                    })()}
                    ...
                    {device.type === "除濕機" && (
                      <div className="space-y-3">
                        ...
                        <div><label className="text-xs text-mute">電源</label><div className="mt-1 flex gap-2"><button onClick={() => sendDehumidifierCommand(device.name, { power: true })} disabled={dhPending !== null} className={`rounded-lg px-4 py-1.5 text-sm font-medium transition-colors ${isFailed("power", true) ? "bg-warm text-white animate-pulse" : isPending("power", true) ? "bg-amber-500 text-white animate-pulse" : device.power ? "bg-cool text-white" : "bg-elevated text-soft hover:bg-mute/20"}`}>ON</button><button onClick={() => sendDehumidifierCommand(device.name, { power: false })} disabled={dhPending !== null} className={`rounded-lg px-4 py-1.5 text-sm font-medium transition-colors ${isFailed("power", false) ? "bg-warm text-white animate-pulse" : isPending("power", false) ? "bg-amber-500 text-white animate-pulse" : device.power === false ? "bg-warm text-white" : "bg-elevated text-soft hover:bg-mute/20"}`}>OFF</button></div></div>
                        <div><label className="text-xs text-mute">模式</label><div className="mt-1 flex flex-wrap gap-2">{options.dehumidifier.modes.map((m) => (<button key={m.value} onClick={() => sendDehumidifierCommand(device.name, { mode: m.value })} disabled={dhPending !== null} className={btnClass("mode", m.value, device.mode === m.label)}>{m.label}</button>))}</div></div>
                        <div><label className="text-xs text-mute">目標濕度</label><div className="mt-1 flex flex-wrap gap-2">{options.dehumidifier.humidity.map((h) => (<button key={h} onClick={() => sendDehumidifierCommand(device.name, { humidity: h })} disabled={dhPending !== null} className={btnClass("humidity", h, String(device.targetHumidity) === `${h}%`)}>{h}%</button>))}</div></div>
                      </div>
                    )}
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {(device.buttons ?? "").split(",").map(b => b.trim()).filter(Boolean).map((btn) => (
                            <button key={btn} onClick={() => sendIrCommand(device.name, btn)} className="rounded-lg bg-elevated px-4 py-2 text-sm font-medium text-soft hover:bg-mute/20 active:bg-mute/30 transition-colors">{btn}</button>
                          ))}
                        </div>
