...
demo 提供 HA 測試空調的合成自動排程可編輯／刪除；不模擬背景計時或實體到期控制。

## 手動排程與版面（v1.55.0）

家電卡展開「排程 → 新增排程」，選擇日期、時間與動作，可管理一次性排程。
HA 空調也支援開關、整數目標溫度、模式與風速，執行路徑為 Dashboard → HB 排程 → HA。
這不會建立／修改 HA 自動化；HB 每 60 秒檢查到期工作，依賴 HB 與 HA 連線。
排程表單統一為共用下拉選單、38px 控制項及收合區塊，收合保留草稿。
頁面預留捲軸空間，避免展開照明色盤時水平位移。無須更新 HA 整合。
...
空調目標一律整數；半度、室溫回饋補償與防黴送風已於 v1.58.0 移除。手動排程與自動關機由 HB 經 HA 執行；
...
**裝置控制**（H1）
- 按房間分群顯示所有可控設備
- 空調：ON/OFF + 溫度調整（步幅 1°C，範圍依後端 options）+ 模式 + 風速 + 送出設定按鈕（dirty 才亮）
- 除濕機：
